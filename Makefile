.PHONY: help validate validate-release smoke bench test report all dry-run-causal dry-run-bi dry-run-reranker autoresearch-smoke autoresearch-controller-dry autoresearch-report autoresearch-validate clean
...
help:
	...
	@echo "  autoresearch-controller-dry  controller status + next-trial plan (stdlib, no GPU)"
	@echo "  autoresearch-report          Pareto/frontier report across saved artifacts (stdlib)"
	@echo "  autoresearch-validate        validate the v8 program configs + run controller tests"
...
autoresearch-controller-dry:
	$(PY) scripts/ar_controller.py status
	$(PY) scripts/ar_controller.py next --dry-run

autoresearch-report:
	$(PY) scripts/ar_report.py --format markdown

autoresearch-validate:
	$(PY) -m unittest tests.test_autoresearch_state tests.test_ar_controller \
		tests.test_autoresearch_mixture_validation tests.test_data_mixture_optimizer \
